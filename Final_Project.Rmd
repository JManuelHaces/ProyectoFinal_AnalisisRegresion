...
```
Conclusion:
- The treshold that we chose is '60', we chose this one because it has the best 'Specificity', with this treshold we can predict better the *Falase Negatives* (We predict that the client will not repurchase and they don't).
...
data$Prob_Predicted <- logistic %>% predict(data, type = "response")

# Making a Label for each case
data$Prob_Predicted <- case_when(
    data$Prob_Predicted >= treshold_selec ~ "High Probability",
    data$Prob_Predicted <= 0.15 ~ "Low Probability",
...
```{r}
data_merge <- data_merge %>% select(c(id, age_gen, market, ))
data_fin <- merge(data_fin, data_merge, by="id",)
data_fin
```
